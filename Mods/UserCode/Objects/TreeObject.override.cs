...
// Created by SplitMilky on 2024-04-14. Version 1.0.1
namespace Eco.Mods.Organisms
{
    ...
    public class TreeEntity : Tree, IDamageable, IHasInteractions
    {
        ...
        /// These are the two mod variables that control the debris removal system.
        /// debrisRemoveLevel is the level at which the player can remove debris.
        /// removeDebris is the boolean that enables or disables the debris removal system.
        /// </summary>
        bool removeDebris = true;    //Enable/Disable auto-debris removal
        int debrisRemoveLevel = 0;  //Level at which player can remove debris
                ...
                for (var z = -radius; z <= radius; z++)
                {
                    ...
                    if (!aboveblock.Is<Solid>())
                    {
                        ...
                        if (hitblock.Is<Solid>() && World.GetBlock(abovepos).Is<Empty>() && RandomUtil.Value < this.Species.ChanceToSpawnDebris)
                        {
                            ...
                            /// This code instead of spawning debris, adds resources to the player's inventory if they have the required skill level.
                            /// If they don't have the required skill level, it will spawn debris instead.
                            /// </summary>

                            var loggingSkill = player.User.Skillset.GetSkill(typeof(LoggingSkill));
                            if (loggingSkill != null && loggingSkill.Level >= this.debrisRemoveLevel)
                            {
                                var changes = InventoryChangeSet.New(player.User.Inventory, player.User);
                                this.Species.DebrisResources.ForEach(x => changes.AddItems(x.Key, x.Value.RandInt));
                                changes.TryApply();
                            }
                            else
                            {
